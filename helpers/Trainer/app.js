...
const boundary = require('./helpers/boundary');
...
function trainSingle (data, callback){
    ...
    trainer(data.genre, function(jsonGenre){
        console.log("Genre training complete")
        trainer(data.activity, function(jsonActivity){
            console.log("Activity training complete")
            callback([jsonGenre, jsonActivity]);
        });
    });
        ...
        if (process.argv[2] === "learnPlaylist") {
            ...
            async.eachOfSeries(trackDictionary, function (dictionaryValue, mainKey, dictionaryCallback) {
                async.eachOfSeries(dictionaryValue.uriList, function (uriValue, uriKey, uriCallback) {
                    ...
                    grabFeatures(dictionaryValue.category, uriValue, function(data){
                        console.log(memory)
